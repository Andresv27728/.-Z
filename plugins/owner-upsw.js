// ─────────────── ⋆⋅☆⋅⋆ ───────────────
// 🌊🦈 GuraBot - Subir a Estado (GC Only) 📤
// "Glub glub~ ¡Vamos a compartir esto en el arrecife~!" 🐟
// ─────────────── ⋆⋅☆⋅⋆ ───────────────
...
let handler = async (m, { conn, command, args }) => {
  ...
  if (m.quoted && m.quoted.mtype) {
    ...
    if (mtype === "audioMessage") {
      ...
      throw "❌🐋 Tipo de archivo no válido, glub~!";
    }
    ...
    await conn
      ...
      .then((res) => {
        conn.reply(m.chat, `✅🦈 Sube con éxito a estado: *${type}*`, res);
      })
      .catch(() => {
        conn.reply(m.chat, `⚠️🐟 Falló la subida de: *${type}*`, m);
      });
  } else {
    throw "❌🐠 No se encontró ningún archivo o mensaje para subir!";
  }
};
...
// 🎨 Función para color random en los estados
...
// ─────────────── ⋆⋅☆⋅⋆ ───────────────
// Gawr Gura: "Ehehe~ ¡tu estado ya está nadando por el océano! 🐠"
// ─────────────── ⋆⋅☆⋅⋆
